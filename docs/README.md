...
| [package-versioning.md](package-versioning.md) | Version-controlling what nix installs — the two-granularity model: committed `manifests/` (version-level, in every Renovate PR diff, CI-gated) + `nvd`-at-switch / `whatchanged` (store-path-level, catches same-version rebuilds). Led by the xdph case study. |
...
| [opencode.md](opencode.md) | opencode at parity with the Claude Code setup — why the config is GENERATED (`nix run .#gen-opencode`) rather than written, the verified Claude↔opencode incompatibilities (`${VAR}` vs `{env:VAR}`, no glob in `skills.paths`, no hooks), the stable-vs-volatile split behind the skill symlink farm, and the deliberate parity gaps. |
